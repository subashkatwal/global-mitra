    ...
    return (
      <div className="min-h-screen bg-[#F3F6FA] flex items-center justify-center px-4">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <button
            onClick={onBack}
            className="px-5 py-2 rounded-lg bg-[#1B4332] text-white text-sm font-semibold hover:bg-[#2D6A4F] transition-colors"
          >
            Go Back
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#F3F6FA]">
      ...
      <div className="relative w-full h-[300px] sm:h-[400px] lg:h-[480px] bg-gray-200 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 sm:px-10 pb-7">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-2 drop-shadow">
            {d.name}
          </h1>
          {location && (
            ...
            >
              {[
                ...
                },
                {
                  icon: DollarSign,
                  label: "Avg. Cost",
                  value: `NPR ${Math.round(d.averageCost)}`,
                },
              ].map(({ icon: Icon, label, value }) => (
                <div
                  key={label}
                  className="bg-white rounded-2xl border border-gray-100 px-4 py-4 flex items-center gap-3 shadow-sm"
                >
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {d.famousLocalItems.map((item) => (
                    <span
                      key={item}
                      className="px-4 py-2 rounded-full border border-gray-200 text-sm text-gray-600 bg-white hover:border-gray-300 transition-colors"
                    >
                      {item}
                    </span>
                  ))}
              ...
              <InfoRow
                ...
                badge={
                  <Badge
                    label={d.crowdLevel}
                    colorClass={
                      CROWD_COLOR[d.crowdLevel] ?? "bg-gray-100 text-gray-600"
                    }
                  />
                }
              />
