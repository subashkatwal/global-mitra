...
import { Plus, Search, Check, X, Eye, Pencil, Trash2, Save, Loader2 } from 'lucide-react';
...
function UserFormModal({
  ...
  const handleSubmit = async () => {
    if (!form.fullName || !form.email) { setErr('Full name and email are required.'); return; }
    ...
    try {
      ...
      else      await apiFetch('/profile/users',             { method: 'POST',  body: JSON.stringify({ ...body, password: form.password }) });
      onSaved();
    ...
    <Modal title={user ? 'Edit User' : 'Add User'} onClose={onClose}>
      ...
      <div className="space-y-4">
        <FormField label="Full Name" req>
          <input value={form.fullName} onChange={e => set('fullName', e.target.value)} placeholder="Jane Doe" className={inputCls} style={inputStyle} />
        </FormField>
        <FormField label="Email" req>
          <input value={form.email} onChange={e => set('email', e.target.value)} type="email" placeholder="jane@example.com" className={inputCls} style={inputStyle} />
        </FormField>
        ...
        {!user && (
          <FormField label="Password" req>
            <input value={form.password} onChange={e => set('password', e.target.value)} type="password" placeholder="Min 8 characters" className={inputCls} style={inputStyle} />
          </FormField>
        )}
      </div>
      <div className="flex gap-3 mt-6">
        ...
        <button onClick={onClose} className="px-5 py-2.5 border rounded-xl text-sm font-semibold hover:bg-gray-50 transition-colors" style={{ borderColor: T.border, color: T.textSub }}>
          Cancel
        </button>
      </div>
    </Modal>
  ...
  return (
    <Modal title="User Details" onClose={onClose}>
      <div className="space-y-0">
        {rows.map(([k, v]) => (
          <div key={k} className="flex items-center justify-between py-2.5 border-b last:border-0" style={{ borderColor: T.borderSm }}>
...
export function UsersPage({ toast }: { toast: ToastFn }) {
  ...
  const roleBadge = (role: string) =>
    role === 'ADMIN' ? 'red' : role === 'GUIDE' ? 'green' : 'blue';
  ...
  return (
    <div>
      ...
      <AnimatePresence>
        {modal === 'add' && (
          <UserFormModal onClose={() => setModal(null)} onSaved={() => { setModal(null); toast('User added!', 'success'); load(); }} />
        )}
        {modal === 'edit' && selected && (
          <UserFormModal user={selected} onClose={() => setModal(null)} onSaved={() => { setModal(null); toast('User updated!', 'success'); load(); }} />
        )}
        {modal === 'view' && selected && (
          <UserDetailModal user={selected} onClose={() => setModal(null)} />
        )}
        {confirmId && (
          <Confirm msg="Delete this user? This cannot be undone." onConfirm={() => handleDelete(confirmId)} onCancel={() => setConfirmId(null)} />
        )}
      </AnimatePresence>
    </div>
  );
