...
import {
  Shield, ChevronLeft, BadgeCheck, Clock, X,
...
export function GuideDashboardPage({ onNavigate }: GuideDashboardPageProps) {
  ...
  const photoUrl   = resolvePhotoUrl(guide.user?.photo);
  const joinDate   = new Date(guide.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
  const lastUpdate = new Date(guide.updatedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
  ...
  return (
    <div className="min-h-screen bg-[#F2F5F3] pt-[68px]">
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
        ...
        {/* Back */}
        <motion.button {...stagger(0)} onClick={() => onNavigate('guide')}
          className="flex items-center gap-1.5 text-sm font-semibold text-[#2D6A4F] hover:text-[#1B4332] transition-colors">
            ...
            <div className="flex items-end gap-4 mb-5">
              <div className="relative flex-shrink-0">
                {photoUrl ? (
                  ...
                  <div className="w-[72px] h-[72px] rounded-2xl ring-4 ring-white shadow-md flex items-center justify-center text-xl font-bold text-white"
            ...
            {stats.map((stat, i) => {
              ...
              return (
                <motion.div key={stat.label}
                  ...
                  className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-col items-center gap-2 hover:shadow-md transition-shadow cursor-default"
                >
