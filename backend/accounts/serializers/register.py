    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            PasswordResetOTP.objects.create(
                user=user, otp_hash=otp_hash, purpose="registration"
            )
...
class ResendRegistrationOTPSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        PasswordResetOTP.objects.filter(
            user=user, purpose="registration", isUsed=False
        ).update(isUsed=True)
        otp = generate_otp()
        otp_hash = hash_otp(otp)
        PasswordResetOTP.objects.create(
...
class GuideProfileCompleteSerializer(serializers.Serializer):
    ...
    licenseIssuedBy = serializers.CharField(
        required=False, default="Nepal Tourism Board"
    )
    ...
    def update(self, instance, validated_data):
        ...
        instance.licenseNumber = validated_data.get(
            "licenseNumber", instance.licenseNumber
        )
        instance.licenseIssuedBy = validated_data.get(
            "licenseIssuedBy", instance.licenseIssuedBy
        )
