...
class PostListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return (
            ...
            .prefetch_related("comments", "bookmarkedBy", "likedBy", "sharedBy")
            .order_by("-createdAt")
        )
...
class PostDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Post.objects.select_related("user").prefetch_related(
        "comments", "bookmarkedBy", "likedBy", "sharedBy"
    )
    serializer_class = PostSerializer
    parser_classes = [MultiPartParser, FormParser]
...
@extend_schema(
    tags=["Posts"],
    description="Toggle like on a post. First call likes it, second call removes the like.",
    parameters=[
        OpenApiParameter(
            name="pk",
    ...
    responses={
        200: OpenApiResponse(description="Like removed."),
...
class PostLikeView(GenericAPIView):
    ...
    def post(self, request, pk):
        ...
        if post.likedBy.filter(pk=user.pk).exists():
...
class MyBookmarksView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return (
            Post.objects.filter(bookmarkedBy__user=self.request.user)
            .select_related("user")
            .prefetch_related("comments", "bookmarkedBy", "likedBy", "sharedBy")
...
class PostShareView(GenericAPIView):
    ...
    def get(self, request, pk):
        ...
        platform_stats = list(
            shares.values("platform").annotate(count=Count("id")).order_by("-count")
        )
        return Response(
            {
...
@extend_schema(
    tags=["Comments"],
    description="Toggle like on a comment. First call likes it, second call removes the like.",
    parameters=[
        OpenApiParameter(
            name="pk",
    ...
    responses={
        200: OpenApiResponse(description="Like removed."),
        201: OpenApiResponse(description="Comment liked."),
...
class CommentLikeView(GenericAPIView):
    ...
    def post(self, request, pk):
        ...
        if comment.likedBy.filter(pk=user.pk).exists():
...
class CommentDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Comment.objects.select_related("user").prefetch_related("likedBy")
    serializer_class = CommentSerializer
    parser_classes = [MultiPartParser, FormParser]
    http_method_names = ["get", "patch", "delete"]
