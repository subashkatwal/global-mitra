...
# Configuration
# --------------------------
BASE_URL = "http://localhost:8000/reports"  # Replace with your backend URL
...
# --------------------------
test_incidents = [
    {"title": "Tree fallen on road", "description": "Large tree blocking main street", "location": "Kathmandu"},
    {"title": "Car accident", "description": "Minor collision near city park", "location": "Bhaktapur"},
]
...
def test_user(token, username):
    ...
    if incident_ids:
        resp = requests.patch(f"{BASE_URL}/incidents/{incident_ids[0]}", json={"title": "Updated title"}, headers=headers)
        print_response(resp)

        # Admin-only actions: verify/reject
